...
<template>
  ...
  <nos-paragraphs :decent="true">
    <template #text>
      <div-paragraph> Hier können alters-/gewichtsabhängie Parameter zu Kindern ermittelt werden. </div-paragraph>
    </template>
  </nos-paragraphs>
  ...
  <nos-header
    :decent="true"
    title="Gewicht schätzen oder Eingeben"
    icon="$weight"
  />
  <nos-btn-group
    v-model="step1"
        ...
        <div>
          ...
          <div class="nos-forms-textfield-inline">
            <v-text-field
              ...
              type="tel"
              @keypress="filterMonths($event, true)"
            />
          </div>
        </div>
      </template>
      ...
      <template v-else-if="step3 === 'estimate'">
        ...
        >
          <v-select
            ...
            hide-details="auto"
            variant="solo"
            @update:model-value="ageGroupSelectChanged"
          />
        </div>
      </template>
    </template>
  ...
  <template v-if="weightValue > 0">
    ...
    <nos-table>
      <nos-row-result :color="weightResult.broselow.color">
        <template #heading>
          {{ weightResult.broselow.title }}
        </template>
        <template #text>
          {{ weightResult.broselow.range }}
        </template>
      </nos-row-result>
      <nos-row-caption>
        <template #caption>
        ...
        <template #content>
          <span-mono>{{ weightResult.vitals.pulse }}</span-mono>/min
        </template>
      </nos-row-caption>
      <nos-row-caption>
        <template #caption>
        ...
        <template #content>
          <div-u> Durchschnittsdruck </div-u>
          <div>
            <span-mono>{{ weightResult.vitals.rrNorm }}</span-mono>mmHg
          </div>

          <v-divider class="my-2" />

          <div-u class="text-blue-darken-3">
            Untere Warnschwelle
          </div-u>
          <div class="text-blue-darken-3">
            <span-mono>{{ weightResult.vitals.rrLow }}</span-mono>mmHg
          </div>

          <v-divider class="my-2" />

          <div-u class="text-red-darken-4">
            Obere Warnschwelle
          </div-u>
          <div class="text-red-darken-4">
            <span-mono>{{ weightResult.vitals.rrHigh }}</span-mono>mmHg
          </div>
        </template>
      </nos-row-caption>
      <nos-row-caption>
        ...
        <template #content>
          <div>
            <span-mono>{{ weightResult.vitals.breath }}</span-mono>/min
          </div>
        </template>
      </nos-row-caption>
    </nos-table>
    <v-btn
      :to="'/content/calc-doses?weight=' + weightValue"
      color="error"
      size="large"
      prepend-icon="$alarmPanelOutline"
      class="mx-2 my-2"
    >
      Dosisrechner mit {{ weightValue }}kg aufrufen
    </v-btn>
  </template>
</template>
...
export default {
  ...
  watch: {
    ...
    ageYears (newValue, oldValue) {
      if (newValue === '0') {
        ...
        const age = parseInt(!newValue ? '0' : newValue) + parseInt(!this.ageMonths ? '0' : this.ageMonths) / 12
        this.weightValue = this.getFromAge(age)
      }
    },
    ageMonths (newValue, oldValue) {
      const age = parseInt(!this.ageYears ? '0' : this.ageYears) + parseInt(!newValue ? '0' : newValue) / 12
      this.weightValue = this.getFromAge(age)
    },
  ...
  methods: {
    ...
    filterYear (e) {
      const char = String.fromCharCode(e.keyCode)
      if (/^[0-9]+$/.test(char)) {
        const year = parseInt(this.ageYears + char)
    ...
    filterMonths (e) {
      const char = String.fromCharCode(e.keyCode)
      if (/^[0-9]+$/.test(char)) {
        const months = parseInt(this.ageMonths + char)
    ...
    filterHeight (e) {
      const char = String.fromCharCode(e.keyCode)
      if (/^[0-9.]+$/.test(char)) {
        const number = parseFloat(this.height + char)
        if (isNaN(number)) {
          e.preventDefault()
          return
        }
    ...
    getFromHeight (height) {
      const under = this.calcTable.findLast((e) => height >= e.height)
      const over = this.calcTable.find((e) => height < e.height)
      if (under && !over) {
        return under.weight
      } else if (!under && over) {
        return over.weight
    ...
    getFromAge (age) {
      ...
      const under = this.calcTable.findLast((e) => age >= e.age)
      const over = this.calcTable.find((e) => age < e.age)
      if (under && !over) {
        return under.weight
      } else if (!under && over) {
        return over.weight
